...
namespace Metalitix.Scripts.Dashboard.Editor
{
    ...
    {
        private MetalitixScene _targetMetalitixScene;
        private MetalitixScene _previewMetalitixScene;
        ...
        private async void OnEnable()
        {
            ...
            if (!FindMetalitixSceneInActiveScene()) return;
        ...
        private void InitializePreviewScene()
        {
            ...
            _previewRenderer = new ScenePreviewRenderer(60, -10, editorSettings.DashboardBackgroundColor, editorSettings.GraphicsFormatForScenePreview, position.width, position.height);
            ...
            InitializeContainer();
            _metalitixVisualizer.Initialize(_previewMetalitixScene.gameObject, _previewRenderer.Scene);
        ...
        private void InitializeContainer()
        {
            ...
            _dashboardContainer.Initialize(_previewRenderer.Scene, _dashboardCamera, _previewMetalitixScene);
            _previewRenderer.MoveWithoutInstance(_dashboardContainer.gameObject);
        }

        private void MoveModelViewer()
        {
            var instanceVisualizer = _previewRenderer.MoveToTheSceneWithInstance(_targetMetalitixScene.gameObject);
            _previewMetalitixScene = instanceVisualizer.GetComponent<MetalitixScene>();
            instanceVisualizer.transform.parent = _metalitixVisualizer.GetWrapperTransform();
        }

        private bool FindMetalitixSceneInActiveScene()
        {
            var metalitixScene = FindObjectOfType<MetalitixScene>();

            if (metalitixScene == null)
            {
                MetalitixDebug.LogError(this, MetalitixEditorLogs.NoFoundDashboardModelInScene);
                return false;
            }

            _targetMetalitixScene = metalitixScene;
            return true;
        }
