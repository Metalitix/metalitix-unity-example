...
namespace Metalitix.Scripts.Preview.Editor
{
    public class ScenePreviewRenderer
    {
        ...
        public ScenePreviewRenderer(float fov, float zPosMultiplier, Color backgroundColor, GraphicsFormat graphicsFormat, float width, float height)
        {
            ...
            if (!_scene.IsValid())
                throw new InvalidOperationException(MetalitixEditorLogs.PreviewSceneError);

            InitializeScene(fov, zPosMultiplier, backgroundColor, graphicsFormat);
        }

        private void InitializeScene(float fov, float zPosMultiplier, Color backgroundColor, GraphicsFormat graphicsFormat)
        {
            ...
            _renderTexture = new RenderTexture(Mathf.RoundToInt(_curAspect * _renderTextureHeight),
                _renderTextureHeight, 32, graphicsFormat);
            ...
            _renderCamera.transform.position = Vector3.forward * zPosMultiplier;
