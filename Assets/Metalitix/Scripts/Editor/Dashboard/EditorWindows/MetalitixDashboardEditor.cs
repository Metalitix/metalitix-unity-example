...
using Metalitix.Scripts.Editor.Dashboard.Requests.DataGetters;
using Metalitix.Scripts.Editor.Dashboard.Requests.DataPatchers;
using Metalitix.Scripts.Editor.Dashboard.Roles.Processes;
using Metalitix.Scripts.Editor.Dashboard.Settings;
using Metalitix.Scripts.Editor.EditorTools;
using Metalitix.Scripts.Editor.EditorTools.DockTools;
...
using Metalitix.Scripts.Runtime.Dashboard.Algorithms;
...
using Metalitix.Scripts.Runtime.Dashboard.Visualizers;
...
using SessionState = Metalitix.Scripts.Runtime.Dashboard.DashboardSystem.States.DashboardStates.SessionState;
...
namespace Metalitix.Scripts.Editor.Dashboard.EditorWindows
{
    public class MetalitixDashboardEditor : EditorWindow
    {
        ...
        private const int ViewerWindowWidth = 1000;
        private const int ViewerWindowHeight = 590;
        ...
        private static void ShowWindow()
        {
            ...
            _editor.maxSize = new Vector2(ViewerWindowWidth,
                ViewerWindowHeight);
        ...
        private async void OnEnable()
        {
            ...
            {
                EditorUtility.DisplayDialog("App keyCode missing", "Please check is you appKey is not empty", "Ok");
                return;
            }
            ...
            {
                EditorUtility.DisplayDialog("APIKey Not Valid", "Please check if the appKey you entered is correct.", "Ok");
                return;
            }
        ...
        {
            _previewRenderer = new ScenePreviewRenderer(60, new Color32(7,6,26,1));
            ...
            {
                throw new NullReferenceException("Visualizer not found in resources folder");
            }
            ...
            InitializeContainer();
            _metalitixVisualizer.Initialize(_targetGameObject, _previewRenderer.Scene);
        ...
        private void MoveModelViewer()
        {
            ...
            _targetGameObject = goArray
                .Where(go => go.TryGetComponent<MetalitixDashboardModel>(out var component))
                .Select(go => go).First();
            
            var instanceVisualizer = _previewRenderer.MoveToTheSceneWithInstance(_targetGameObject);
        ...
        private void DrawLoginPage()
        {
            ...
            Rect labelRect = new Rect(position.width / 2 - labelWidth / 2, position.height / 2 - labelHeight / 2, labelWidth,
                labelHeight);
            GUIContent labelContent = new GUIContent("Please, login into your account!");
        ...
        private void DrawHeatMapSettingsButton()
        {
            ...
            var content = new GUIContent("", MetalitixStartUpHandler.Settings);
        ...
        private void DrawViewerButtons()
        {
            ...
            _datePeriod.CurrentValue = GUI.Toolbar(new Rect(position.width - 626, 0, 500, 30), _datePeriod.CurrentValue, _periodToolbarStrings);

            var isDisabled = _filterWrappers.Count == 0;

            EditorGUI.BeginDisabledGroup(isDisabled);
            DrawDropdown(new Rect(position.width - 126, 0, 125, 30), new GUIContent("Filters"));
        ...
        private async void BackendCalculation()
        {
            ...
            {
                EditorUtility.DisplayDialog("Warning", "HeatMap UpdatedData for that period not calculated yet!", "Ok");
                return;
            }
